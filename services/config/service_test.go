...
func OpenNewService(t *testing.T, testConfig interface{}, updates chan<- config.ConfigUpdate) (*config.Service, *httpdtest.Server) {
	c := config.NewConfig()
	service := config.NewService(c, testConfig, diagService.NewConfigOverrideHandler(), updates)
	service.StorageService = storagetest.New(t, diagService.NewStorageHandler())
