...
func newDerivativeNode(et *ExecutingTask, n *pipeline.DerivativeNode, d diagnostic.Diagnostic) (*DerivativeNode, error) {
	dn := &DerivativeNode{
...
func (n *DerivativeNode) derivative(prev, curr models.Fields, prevTime, currTime time.Time) (float64, bool, bool) {
	...
	if elapsed == 0 {
		...
		n.diagnostic.Diag(
			"level", "error",
			"msg", "cannot perform derivative elapsed time was 0",
		)
		return 0, true, false
	}
	diff := f1 - f0
